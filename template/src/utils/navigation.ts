 ...
 */
export function useNavigationPersistence(storage: Storage) {
  ...
  const onNavigationStateChange = (state: NavigationState | undefined) => {
    ...
    if (state !== undefined) {
      ...
      storage.save(NAVIGATION_PERSISTENCE_KEY, state).catch(err => {
  ...
  const restoreState = async () => {
    try {
      const state = (await storage.load(NAVIGATION_PERSISTENCE_KEY)) as
